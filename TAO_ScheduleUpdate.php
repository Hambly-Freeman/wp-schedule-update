<?php
/**
 * Plugin Name: WP Schedule Update
 * Description: Allows you to plan changes on any post type
 * Author: Originally TAO Digital, forked by Hambly Freeman
 * Author URI: https://hamblyfreeman.com/
 * Version: 1.15
 * License: MIT
 * Text Domain: wp-schedule-update
 */
...
class TAO_ScheduleUpdate {
  ...
  public static function init() {
    ...
    foreach ($pt as $type) {
      add_action('manage_edit-' . $type->name . '_columns', ['TAO_ScheduleUpdate', 'manage_pages_columns']);
      add_filter('manage_' . $type->name . '_posts_custom_column', ['TAO_ScheduleUpdate', 'manage_pages_custom_column'], 10, 2);
      add_action('add_meta_boxes', ['TAO_ScheduleUpdate', 'add_meta_boxes_page'], 10, 2);
    }
  }
...
//    echo '<style> #duplicate-action, #delete-action, #minor-publishing-actions, #misc-publishing-actions, #preview-action {display:none;} </style>'; // WPCS: XSS okay.
    ...
    wp_enqueue_script(self::$_tao_publish_status . '-datepicker.js', plugins_url('js/publish-datepicker.js', __FILE__), ['jquery-ui-datepicker']);
    ...
    $df = str_replace(
      ['d', 'j', 'S', 'l', 'D', 'm', 'n', 'F', 'M', 'Y', 'y', 'c', 'r', 'U'],
      ['dd', 'd', '', 'DD', 'D', 'mm', 'm', 'MM', 'M', 'yy', 'y', 'yy-mm-dd', 'D, d M yy', '@'],
      $df
    );
    ...
    add_meta_box('meta_' . self::$_tao_publish_status, self::$_tao_publish_metabox, ['TAO_ScheduleUpdate', 'create_meta_box'], $post_type, 'side');
  }
  ...
  public static function create_meta_box($post) {
    ...
    <label class="screen-reader-text"
           for="<?php echo esc_attr($metaname); ?>"><?php esc_html_e('Releasedate', 'tao-scheduleupdate-td'); ?></label>
    <input type="hidden"
           name="<?php echo esc_attr($metaname); ?>"
           id="<?php echo esc_attr($metaname); ?>"
           value="<?php echo esc_attr($date2); ?>"/>
    <input type="text"
           class="widefat"
           name="<?php echo esc_attr($metaname); ?>_display"
           id="<?php echo esc_attr($metaname); ?>_display"
           value="<?php echo esc_attr($date); ?>"/>
    <p>
      <strong><?php esc_html_e('Time', 'tao-scheduleupdate-td'); ?></strong>
    </p>
    <label class="screen-reader-text"
           for="<?php echo esc_attr($metaname); ?>_time"><?php esc_html_e('Time', 'tao-scheduleupdate-td'); ?></label>
    ...
    <input type="hidden"
           name="tao_added_minutes"
           id="tao_used_gmt"
           value="<?php echo esc_attr($gmt_hour >= 0 ? '+' : '-');
  ...
  public static function prevent_status_change($new_status, $old_status, $post) {
    ...
    if ($old_status === self::$_tao_publish_status && 'trash' !== $new_status) {
      remove_action('save_post', ['TAO_ScheduleUpdate', 'save_meta'], 10);

      $post->post_status = self::$_tao_publish_status;
      $u = wp_update_post($post, true);

      add_action('save_post', ['TAO_ScheduleUpdate', 'save_meta'], 10, 2);
    } elseif ('trash' === $new_status) {
      wp_clear_scheduled_hook('tao_publish_post', [
        'ID' => $post->ID,
      ]);
    } elseif ('trash' === $old_status && $new_status === self::$_tao_publish_status) {
      wp_schedule_single_event(get_post_meta($post->ID, self::$_tao_publish_status . '_pubdate', true), 'tao_publish_post', [
        'ID' => $post->ID,
      ]);
    }
  }
    ...
    if ($post->post_status === self::$_tao_publish_status || get_post_meta($post_id, self::$_tao_publish_status . '_original', true)) {
      ...
      if (isset($_POST[$pub]) && isset($_POST[$pub . '_time_hrs']) && isset($_POST[$pub . '_time_mins']) && !empty($_POST[$pub])) {
        ...
        if (!$stampchange || TAO_ScheduleUpdate_Options::get('tsu_nodate') === 'publish') {
          ...
          wp_schedule_single_event($stamp, 'tao_publish_post', [
            'ID' => $post_id,
          ]);
        }
      }
...
add_action('save_post', ['TAO_ScheduleUpdate', 'save_meta'], 10, 2);
add_action('tao_publish_post', ['TAO_ScheduleUpdate', 'cron_publish_post']);

add_action('wp_ajax_load_pubdate', ['TAO_ScheduleUpdate', 'load_pubdate']);
add_action('init', ['TAO_ScheduleUpdate', 'init'], PHP_INT_MAX);
add_action('admin_action_workflow_copy_to_publish', ['TAO_ScheduleUpdate', 'admin_action_workflow_copy_to_publish']);
add_action('admin_action_workflow_publish_now', ['TAO_ScheduleUpdate', 'admin_action_workflow_publish_now']);
add_action('transition_post_status', ['TAO_ScheduleUpdate', 'prevent_status_change'], 10, 3);

add_filter('display_post_states', ['TAO_ScheduleUpdate', 'display_post_states']);
add_filter('page_row_actions', ['TAO_ScheduleUpdate', 'page_row_actions'], 10, 2);
add_filter('post_row_actions', ['TAO_ScheduleUpdate', 'page_row_actions'], 10, 2);
add_filter('manage_pages_columns', ['TAO_ScheduleUpdate', 'manage_pages_columns']);
add_filter('page_attributes_dropdown_pages_args', ['TAO_ScheduleUpdate', 'parent_dropdown_status']);
